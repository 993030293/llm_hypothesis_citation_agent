...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the official DeepScientist 15-case citation-audit campaign.")
...
def main_with_args(args: argparse.Namespace) -> int:
    ...
    if not args.no_copy_submission:
        target = ROOT / "submission" / "deepscientist_15x_campaign"
        if target.exists():
...
def load_real_cases(manifest_path: Path) -> list[dict[str, Any]]:
    ...
    for case in manifest.get("cases", []):
        ...
        cases.append(case)
        seen.add(case_id)
    if CLIP_CASE["case_id"] not in seen:
        cases.append(CLIP_CASE)
    return cases[:15]
...
def run_case_subprocess(case: dict[str, Any], run_id: str, campaign_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    pdf_path_or_url = case.get("pdf_url") or case.get("pdf_path")
    cmd = [
        sys.executable,
...
def load_case_status(campaign_dir: Path, case: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    status["pdf_path_or_url"] = case.get("pdf_url") or case.get("pdf_path")
...
def render_campaign_report(rows: list[dict[str, Any]], deficiencies: list[str]) -> str:
    ...
    lines = [
        "# Official DeepScientist 15x Campaign Report",
        "",
